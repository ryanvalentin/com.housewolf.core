...
public class SceneReferencePropertyDrawer : PropertyDrawer
{
    ...
    private static Dictionary<GUID, Tuple<int, EditorBuildSettingsScene>> _sceneCache = null;
    public static Dictionary<GUID, Tuple<int, EditorBuildSettingsScene>> SceneCache
    {
        get
        {
            if (_sceneCache == null)
            {
                _sceneCache = new Dictionary<GUID, Tuple<int, EditorBuildSettingsScene>>();
                for (int index = 0; index < EditorBuildSettings.scenes.Length; ++index)
                {
                    _sceneCache.Add(EditorBuildSettings.scenes[index].guid, new Tuple<int, EditorBuildSettingsScene>(index, EditorBuildSettings.scenes[index]));
                }
            }
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        BuildUtils.BuildScene buildScene = BuildUtils.GetBuildScene(selectedObject, SceneCache);
    ...
    private void DrawSceneInfoGUI(Rect position, BuildUtils.BuildScene buildScene, int sceneControlID)
    {
        ...
        using (new EditorGUI.DisabledScope(readOnly))
        {
            ...
            else
            {
                ...
                if (DrawUtils.ButtonHelper(buttonRect, stateString, stateString, EditorStyles.miniButtonLeft, tooltipMsg))
                {
                ...
                if (DrawUtils.ButtonHelper(buttonRect, "Remove...", "Remove", EditorStyles.miniButtonMid, tooltipMsg))
                {
        ...
        if (DrawUtils.ButtonHelper(buttonRect, "Settings", "Settings", EditorStyles.miniButtonMid, tooltipMsg))
        {
        ...
        tooltipMsg = "Refreshes build scene list, do this if you've recently changed the build settings!";
        if (DrawUtils.ButtonHelper(buttonRect, "Refresh...", "Refresh", EditorStyles.miniButtonRight, tooltipMsg))
                RefreshBuildSceneCache();
    }

    private void RefreshBuildSceneCache()
    {
        _sceneCache = null;
    }
    ...
    static private class BuildUtils
    {
        ...
        /// </summary>
        static public BuildScene GetBuildScene(UnityEngine.Object sceneObject, Dictionary<GUID, Tuple<int, EditorBuildSettingsScene>> sceneCache)
        {
            ...
            if (sceneCache.TryGetValue(entry.assetGUID, out Tuple<int, EditorBuildSettingsScene> scene))
            {
                entry.scene = scene.Item2;
                entry.buildIndex = scene.Item1;
            }
